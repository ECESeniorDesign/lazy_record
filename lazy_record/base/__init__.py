...
class Base(Validations, metaclass=BaseMetaClass):
    ...
    def __init__(self, **kwargs):
        ...
        if set(["id", "created_at", "updated_at"]) & set(kwargs):
            raise AttributeError("Cannot set 'id', 'created_at', "
                                 "or 'updated_at'")
        table = Repo.table_name(type(self))
        columns = set(type(self)._attributes()) - {"id"}
        for attr in columns:
    ...
    def __getattr__(self, attr):
        """
        Get and cast +attr+ according to the database schema.
        """
        ...
        if attr in attributes:
            try:
                ...
                if value is not None:
                    return attributes[attr].cast(value)
                else:
                    return None
    ...
    def __setattr__(self, name, value):
        """
        Cast +value+ according to the database schema and assign it
        to the current object.
        """
        if name in ("id", "created_at", "updated_at"):
            raise AttributeError("Cannot set '{}'".format(name))
        elif name in type(self)._attributes():
            if value is not None:
                setattr(self, "_" + name,
                        type(self)._attributes()[name].cast(value))
            else:
    ...
    def from_dict(cls, **kwargs):
        ...
        for attr in set(cls._attributes()) - {"created_at", "updated_at"}:
            delattr(obj, "_" + attr)
    ...
    def update(self, **kwargs):
        ...
        id and object timestamps from being set.
        """
        attributes = set(kwargs.keys())
        attributes = attributes & ((set(type(self)._attributes()) -
                     {"id", "created_at", "updated_at"}) |
                     set(associations.associations_for(type(self))))
    ...
    def _do_destroy(self):
        Repo(self.__table).where(id=self.id).delete()
        for dependent in set(type(self).__dependents__):
    ...
    def _do_save(self):
        ...
        attrs = set(type(self)._attributes()) - {"id"}
        if self.id:
